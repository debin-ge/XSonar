...
`docker-compose.yml` 已通过 `x-bake` 内置 `linux/amd64` 和 `linux/arm64` 两个平台配置。`docker compose build` 会继续按当前宿主机做单架构本地构建；`docker buildx bake` 会按多架构配置生成并推送 manifest list。推送后，`amd64` 和 `arm64` 机器都会自动拉取匹配的变体，不需要在服务端手工指定镜像平台。
